...
void readme()
{
	...
	cout<<"This code is modified to take 3 image feeds. It uses nonfree library (see about that)."<<endl;
}
...
i++;
cout<<i<<endl;
	//Mat image1 = imread("../data/S1.jpg");
	//cap1 >> image1; // get a new frame from camera
	Mat image1 = imread("output.jpg");
	cvtColor(image1, gray_image1, COLOR_BGR2GRAY);

	//Mat image2= imread("../data/S2.jpg");
	//cap1 >> image2; // get a new frame from camera
	Mat image2;
	vcap>>image2;
	if(i>121){
...
/*
  ...
  cv::imwrite("Result123new.jpg",img4);
	// waitKey(0);
